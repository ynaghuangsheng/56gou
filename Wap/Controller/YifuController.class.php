<?php
class YifuController extends Controller {
	...
	public function index(){
        ...
        //模板赋值
        $this->assign('index','yifu');//设置导航游标
        $this->assign('tag','index');//设置导航游标
        $this->assign('ajax_url','/m/ajax?cid=1');//设置加载商品地址
	...
	public function tag(){
        ...
        $this->assign('ajax_url','/m/ajax?cid=1&tag='.urlencode($tag));//设置加载商品地址
        $this->assign('list',$list);unset($list);
		$this->display('index.htpl');
