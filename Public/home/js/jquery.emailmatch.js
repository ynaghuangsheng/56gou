(function($){
	...
	function createMailList(oWrap,sVal,suffix,aEail,nK){ // 创建 候选列表
		...
		var mailList = "<li class='first'><p style='display:block;'>请选择邮箱类型</p></li>";
		...
		if(suffix !='' && State ){
			if(nK == 0){
				mailList += '<li class="current"><strong>'+sVal+'</strong>@'+suffix+'</li>';
			} else {
				mailList += '<li><strong>'+sVal+'</strong>@'+suffix+'</li>';
			}
		}
		if($.isArray(aEail)){
			$.each(aEail, function(i){
				if(State && suffix !=''){
					if(i == (nK-1) ){
						mailList += '<li class="current"><strong>'+sVal+'</strong>@'+aEail[i]+'</li>';	
					} else {
						mailList += '<li><strong>'+sVal+'</strong>@'+aEail[i]+'</li>';	
					}
				} else{
					if(i == nK){
						mailList += '<li class="current"><strong>'+sVal+'</strong>@'+aEail[i]+'</li>';	
